...
int main() {
  ...
  mcmf.AddEdge(getAlphaSink(), getGlobalSink(), x1, -alpha*X);			// Modify cost
  mcmf.AddEdge(getPrevAlphaSink(), getGlobalSink(), x2, -(alpha-1)*X);		// Modify cost

  for(int i=1;i<=alpha;i++){
  	for(int j=1;j<=n;j++){
  		if(i<alpha){
	  		for(int l=j+1;l<=n;l++){
	  			mcmf.AddEdge(getFacility(i,j,2), getFacility(i+1,l,1), 1, calc_cost(i-1,j-1));
	  		}
	  	}
  		mcmf.AddEdge(getFacility(i,j,1), getFacility(i,j,2), 1, +X);		// Modify cost
  		mcmf.AddEdge(BalancingSource(j), getFacility(i,j,1), 1, 0);		// Modify cost
  		mcmf.AddEdge(getFacility(i,j,2), BalancingDest(j), 1, -X);
  		if(i==1){
  			mcmf.AddEdge(getSource(), BalancingSource(j), alpha-1, 0);
  			mcmf.AddEdge(BalancingDest(j), getGlobalSink(), alpha-1, 0);
  		}
  	}
  }
